...
Durante lo sviluppo del progetto, abbiamo deciso di adottare alcuni design pattern per migliorare la struttura e la gestione del codice. Tra questi:

\begin{itemize}
    \item \textbf{Singleton Pattern:} è un pattern creazionale che serve a garantire che una certa classe abbia una sola istanza. È utile, ad esempio, per tutte quelle classi che gestiscono risorse comuni nel progetto, come \texttt{ResourceManager}, \texttt{BrowserManager}, ecc.
    ...
    \item \textbf{Factory Pattern:} è un pattern creazionale che permette di delegare la creazione di un oggetto a una funzione o metodo, senza specificare esattamente quale classe verrà istanziata. L’abbiamo usato, ad esempio, per passare parametri tra controller di due viste collegate. Un esempio di codice usato:
...
loader.setControllerFactory(_ -> new ResourceFileController(resource));
    ...
    \item \textbf{DAO (Data Access Object) Pattern:} è un pattern strutturale che separa la logica di accesso ai dati dalla logica di business. Nel nostro caso, abbiamo definito un’interfaccia generica \texttt{DAO<T>} nel package \texttt{model}, che viene implementata da tutte le classi DAO. Ogni entità del modello è rappresentata come una semplice classe Java (POJO), che viene poi gestita dalla relativa implementazione DAO (creazione, modifica, eliminazione, ecc).
\end{itemize}
